...
`examples/minikube/minikube-setup.sh` creates a minikube node with 4 CPUs and 8 GB of memory. On macOS, Docker
and Podman both run containers inside a VM, and that VM needs at least **4 CPUs and 9 GB of memory**:
the node's 8 GB plus headroom.
...
./examples/minikube/minikube-setup.sh

# 2. Deploy the stack into the `obstack` namespace, with the minikube overrides
#    (examples/minikube/values-minikube.yaml) on top of the chart's AWS production defaults
./examples/minikube/deploy-obstack.sh

# 3. Build the sample app image inside minikube and deploy it
./examples/sample-app/build-and-deploy.sh
